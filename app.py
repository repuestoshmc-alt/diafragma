...
def extraer_numero_base(texto: str) -> str:
    """Extrae el bloque numérico principal desde el texto normalizado."""
    coincidencia = re.search(r"\d+", texto or "")
    return coincidencia.group(0) if coincidencia else ""


def coincide_bloque_numerico(modelo_norm: str, numero_base: str) -> bool:
    """Valida coincidencia exacta de bloque numérico evitando superconjuntos como 5100."""
    if not numero_base:
        return False
    patron = rf"(?<!\d){re.escape(numero_base)}(?!\d)"
...
class DiafragmaApp:
    ...
    def buscar_similares(self, consulta_norm: str) -> tuple[list[dict], list[dict]]:
        """Búsqueda estricta sin similitud amplia."""
        exactos, similares = [], []
        numero_base = extraer_numero_base(consulta_norm)
        ...
        for _, fila in self.df.iterrows():
            ...
            if numero_base:
                if coincide_bloque_numerico(modelo_norm, numero_base):
                    similares.append(item)
            elif consulta_norm and (consulta_norm in modelo_norm or consulta_norm in diafragma_norm):
                similares.append(item)
    ...
    def buscar_modelo(self) -> None:
        ...
        if len(resultados) == 0:
            print("Entrando a mostrar_sin_resultados()")
